...
Lugosi, 2017
: El numero esperado de cliques de tamaño $k$ de $\ger[1/2]{n}$ tiende a estar entre dos numeros enteros consecutivos $m$ y $m+1$ a medida que $n$ crece.

Roughgarden, p16
: El numero esperado de cliques de tamaño $k$ es $n^k 2^{-k^2/2}$.

Ese numero vale exactamente $1$ si $k=2\log_2 n$. Es decir, $2\log_2 n$ es el $k$ mas grande para el que esperamos ver al menos un clique de tamaño $k$.
...
::: {.notes}
- "Si se planta un clique de tamaño k, esperamos que los vertices que lo conforman, incrementen su grado en $k-1/2$ (cada vertice tiene que conectarse con otros k − 1 pero en promedio la mitad de esas conexiones ya estaban presentes).
- Grados fuera del clique ~Bin(n-1, 1/2) ancho distribucion √(n log n) (TLC)
...
Las sumas de todas las filas son $\approx$constantes. Entonces, el vector de unos es un $\approx$autovector.

$$
\begin{align}
& \ones = (1, \dots, 1)^\top \\
& \left( E(A) \cdot \ones \right)_i = \sum_j E(A)_{ij} = grado(i) \approx n/2 \\
& E(A) \cdot \ones \approx \frac{n}{2} \ones \\
& u_1 \approx \ones \qquad \lambda_1 \approx n/2
\end{align}
$$
::: {.notes}
- \lambda_1 representa el promedio de conexiones en el grafo
:::
...
& E(A) \cdot w = k w \\
\\
& v_2 \approx w \qquad \lambda_2 \approx k/2
\end{align}
$$
...
Los autovalores del ruido estan acotados por $\sqrt(n)$

El autovalor del clique es $k/2$

Alon funciona cuando la señal del clique es mayor a la del ruido.
...
|-----------|---------------------|---------------------|--------|
| $k < n/2$ | $n/2$ <br> `Ruido`  | $k/2$ <br> `Clique` | $v_2$  |
| $k > n/2$ | $k/2$ <br> `Clique` | $n/2$ <br> `Ruido`  | $v_1$  |
...
La rotacion de los autovectores de $E(A)$ luego de la Perturbacion $P$ depende de la magnitud del ruido y del "eigengap"
