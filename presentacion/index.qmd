...
## Verificacion 2 - El grafico de Manuel
...
graficarSumaConexiones <- function(data, info) {
  ...
  ggplot(df, aes(x = Nodo, y = Suma)) + 
    ...
    labs(
      x = element_blank(),
      y = element_blank(),
      title = sprintf("Suma de conexiones en %s", info)
    )
}

graficarSumaConexiones(colSums(A), "TODA A")
graficarSumaConexiones(colSums(filasEnClique), "filas DENTRO del clique")
graficarSumaConexiones(colSums(filasNoEnClique), "filas FUERA del clique")
```
