...
format:
  revealjs:
    ...
    # 40*60 / 26
    # No cuento algunas slides opcionales. Buscar "uncounted"
    default-timing: 92
    multiplex:
...
# Observar que en el grafo de clique en posicion "trivial" solamente se intercambian los nodos 4 y 6
g <- grafo(CONX_CLIQUE_ALEATORIO)
...
## Matriz de Adyacencia de un $\ger[\half]{n}$ {.centrado}
...
<small>Fuente: [Geogebra](https://www.geogebra.org/graphing/ew6nbcfg)</small>
...
Sea $A$ la matriz de adyacencia de $\ger[p=\half]{n}$
...
$$
E(A) = \begin{pmatrix}
1      & \dots  & 1      & \half  & \dots  & \dots  & \half  \\
\vdots & \ddots & \vdots & \vdots &        &        & \vdots \\
1      & \dots  & 1      & \half  & \dots  & \dots  & \half  \\
\half  & \dots  & \half  & \half  & \dots  & \dots  & \half  \\
\vdots &        & \vdots & \vdots & \ddots &        & \half  \\
\vdots &        & \vdots & \vdots &        & \ddots & \half  \\
\half  & \dots  & \half  & \half  & \dots  & \dots  & \half  \\
\end{pmatrix}
$$
...
::: {.column width="25%"}
Realizacion =<br/><small>Señal + Perturbacion:</small>
$$A = E(A) + P$$
:::
:::
...
\\
& v_1 = \ones \qquad \lambda_1 = \frac{n}{2}
\end{align}
:::
...
::: {.notes}
1. Teoria de Perturbaciones
2. Se pueden calcular los autovalores y autovectores reales y comprobar que son parecidos
3. Se puede aplicar el algoritmo y comprobar que funciona.
:::
...
La rotacion de los autovectores de $E(A)$ luego de la Perturbacion $P$ depende de la magnitud de la perturbacion y del _"eigengap"_

$$\sin\theta_i \leq \frac{2\|P\|}{min_{j\neq i}|\lambda_i-\lambda_j|}$$
<br/>
Conclusion: La rotación de autovectores es tan pequeña que la gran mayoría de los nodos del clique se mantienen en las posiciones superiores del autovector $v_2$
...
graficarRankingVertices(g, v2, k)
```
...
graficarSumaConexiones(colSums(filasEnClique), "DENTRO del clique", k/2, "k/2")
graficarSumaConexiones(colSums(filasNoEnClique), "FUERA del clique", (n-k)/2, "(n-k)/2")
```
<code> n=`r n` k=`r k` </code>
...
## ¿Que pasa de acuerdo a $k$ ? {visibility="uncounted"}
```{r}
n  = 1000
